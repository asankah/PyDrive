  ...
  """
    ...
    """
        ...
        if self.get('title') is None:
            self['title'] = filename
        self.content_mimetype = mimetypes.guess_type(filename)[0]
        if self.get('mimeType') is None:
            self['mimeType'] = self.content_mimetype
    ...
    """
        if self.content_mimetype is None:
            self.content_mimetype = self.get('mimeType', 'application/octet-stream')
        if self.get('mimeType') is None:
            self['mimeType'] = self.content_mimetype

        return MediaIoBaseUpload(self.content,
                                 self.content_mimetype,
                                 resumable=True)
